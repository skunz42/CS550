...
static ssize_t sample_read(struct file *file, char __user *buf, size_t len, loff_t *ppos)
{
    pr_info("Not actually reading anything\n");
    char mybuf[20];
    sprintf(mybuf, "Hello World\n");
    copy_to_user(buf, mybuf, strlen(mybuf)+1);
    return len;
}
